...
from src.parsers.clash2base64 import clash2v2ray
from src.gh_proxy_helper import set_gh_proxy
...
def get_template():
    template_dir = '../public/config-template'  # 配置模板文件夹路径
...
if __name__ == '__main__':
    ...
    if providers.get('config-template'):
        config_template_path = providers['config-template']
        print('选择: \033[33m' + config_template_path + '\033[0m')
        # print ('Mẫu cấu hình sử dụng: \033[33m' + template_list[uip] + '.json\033[0m')
        response = requests.get(providers['config-template'])
        ...
        config_template_path = 'config-template/' + template_list[uip] + '.json'
